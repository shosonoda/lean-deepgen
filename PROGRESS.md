...
| prop:envelope(-restated), cor:envelope-profiles (a)(b), lem:reachable-radius | `Growth/Envelope`（`prop_envelope`, `cor_envelope_profiles`, `lem_reachable_radius`；`geomSum` は `Growth/Defs`） | done |
| lem:ode-scheme-entropy（距離・被覆・積分評価） | `Examples/ODE`（`equalSchemeClass`, `DriftSpace`, `lem_ode_scheme_entropy`, `equalSchemeClass_profile`） | done |
| eq:cot-branch-closed-form, lem:cot-branch-sample | `Examples/ChainOfThought`（`cot_branch_closed_form`, `lem_cot_branch_sample`） | done |
...
- 2026-09-25: 公開版 README（英語，`README.public.md` → 公開時に `README.md` へ置換），Apache-2.0 `LICENSE`，Comparator 証明書ページ（`home_page/comparator.html`，扉ページからリンク；55 定理の対応表と最新の検査結果）を追加（`f503e41`）．公開リポジトリ・サイトへ反映．
- 2026-09-25（新結果）: 原稿に追加された 3 結果を形式化（サブエージェント）．(1) `Growth/Envelope`（新モジュール）: 被覆包絡 prop:envelope(-restated)（`prop_envelope`；語 `F^m` を `F` の `ε/S_m` 被覆中心の合成で覆う，`lem:impl-word-error` の望遠評価を再利用），cor:envelope-profiles (a)(b)（`d_∞` の entropy 評価と `V_k(S)` 評価，`cor_envelope_profiles`），lem:reachable-radius（`lem_reachable_radius`，統一形 `Λ₊^k R + c S_k` と `D_k(S)` の評価）；`S_m(Λ)` は `Growth/Defs.geomSum`（`661c135`, `5850084`）．(2) `Examples/ODE`: 等ステップスキーム類 `E_T(k)`，ドリフトの sup ノルム空間 `DriftSpace K T`，離散 Grönwall による `d_∞(Φ_{s,m},Φ_{s',m}) ≤ T e^{ΛT} ‖s−s'‖_∞`，被覆評価 `N(E_T(k)) ≤ 1 + k N(𝒮_T, ε e^{−ΛT}/T)`（被覆中心を `[0,T]` 外で 0 に延長），積分評価 `equalSchemeClass_profile`（`b8cbee3`）．(3) `Examples/ChainOfThought`: cylinder `InCylinder`（決定可能），閉形式 eq:cot-branch-closed-form，同長 cylinder の互いに素性から `∑_{|u|=j} P̂_n([u]) ≤ 1`，`d_S(f_u, ū_j)² ≤ P̂_n([u])`，標本依存被覆 `N(B(k,F_b), d_S, ε) ≤ 1 + r + ∑_j min{r^j, n, ⌊ε⁻²⌋}` と `V_k(S)` 評価（`de6ee69`）．`lake build` 成功，sorry 0，新定理の `#print axioms` は標準 3 公理のみ．Comparator: 6 定理を Challenge/Solution/config/README に追加しビルド確認（スクリプトの実行は指示により未実施）．原稿との差異（内部/外部被覆数の因子 2，追加仮定）は ToDraft.md「2026-09-25 (new results)」に記録．
- 2026-09-25（続き）: `./script/comparator.sh` を実行し，新規 6 定理を含む `comparator/config.json` の全 61 定理で「Your solution is okay!」（許可公理 `propext`, `Quot.sound`, `Classical.choice` のみ）を確認．`./script/generate.sh --no-pdf` も blueprint web + checkdecls 通過．`home_page/comparator.html`（55→61，新規 6 行を対応表に追加，日付・commit を更新）と `comparator/README.md`（実行待ちの注記を除去）を反映．
- 2026-09-26: 原稿（`../draft-metric-deep`）が枝刈りされた（Lean 側の変更なし，ドキュメントのみ更新）．削除された付録・例: 決定的エントロピー分解（旧 App. E; `thm:rad.decomp.ent.ent` 系），自己写像のコンパクト Arzelà–Ascoli（旧 App. P; `thm:caa`），Guivarc'h 次元（`thm:guivarch-bass` 等，元々未形式化），増大度付録の 7 例（`ex:p1-circle` 等），「4 つの代表的レジーム」付録（`sec:proof.tradeoff` に統合，`tab:tradeoff`/`tab:profiles`/`tab:scorecard`/`tab:cot-summary` は表示式・箇条書きに変更）．`comparator/README.md` と `home_page/comparator.html` の対応表で `thm_rad_decomp_ent_ent`, `thm_rad_decomp_ent_ent_sample`, `thm_caa` の 3 行に「本文未掲載（ライブラリには存置）」の注記を追加．`SUMMARY.md` に削除ラベルの一覧と日付付き注記を追加．`comparator/config.json` とライブラリは変更なし．
- 2026-09-26（ReLU 付録）: 原稿の新付録 `06iclr2027/app-relu.tex`（深層 ReLU ネットワーク）を形式化（サブエージェント；新モジュール `Examples/ReLU`，約 1,580 行）．(1) `lem:relu-layer-covering`: ReLU ブロック `Π_K(V relu(Wx+b)+c)`（`W : E →L[ℝ] ℝ^w`, `V : ℝ^w →L[ℝ] E`，作用素ノルム）のパラメータ Lipschitz 評価 `uniformDist_reluBlock_le` と被覆評価 `log N(F_Λ,d_∞,ε) ≤ p log(1 + C_F/ε)`（`p = 2mw+w+m`，`C_F = 2(2β_W R_K+β_W+β+1) max{β_W,β}`）．一般補題: 有限次元ノルム空間の球の packing 数の体積評価 `packingNumber_closedBall_le`（Haar 測度 `Measure.addHaar` と互いに素な球の測度比較），集合上 Lipschitz な写像の像の外部被覆数 `externalCoveringNumber_image_le_coveringNumber`，`(1 + C/ε)` 型仮定の包絡プロファイル `envelope_profile_one_add`（`7791b4e`）．(2) `prop:relu-regimes` (i)–(iii) 上界: (i) `Λ<1` は `cond_p1_ucont`（`A = K`，`L = 0`）＋望遠評価 `N(F^l,ε) ≤ N(F,(1−Λ)ε)^l` で `N(B(k,F_Λ),ε) ≤ N(K,ε/2) + m(ε) N(F_Λ,(1−Λ)ε)^{m(ε)} < ∞`（全ての `k`）と飽和プロファイル，(ii) `Λ ≤ 1`・`K` コンパクトは `cond_p1_of_nonexpanding`＋包絡 `O(√(kp log k))`，(iii) 包絡 `O(k√(p log Λ))`（`405d8e8`）．(3) (iii) 下界: 計算例の 2 つの expand-and-reset 写像（折れ点は原稿どおり，`η = 1/32`）を区分線型に定義し，幅 4 の ReLU ブロック（`Fin (4+w')` へのゼロ埋め）であること，max–min 表示による 20-Lipschitz 性，`[0,1]` 上の E2 データ（チャンバー `[0,1/4]`,`[3/4,1]`，コア，アンカー 3/8, 5/8，マーカー 1/2，分離 1/8）を検証し `cond_e2_pingpong` から `N(B(k,F_Λ),d_∞,ε) ≥ 2^k`（`ε < 1/16`，`w ≥ 4`，`Λ ≥ 20`，`β_W ≥ 41`，`β ≥ 2`）（`8b61b92`）．`lake build` 成功，sorry 0，主定理の `#print axioms` は標準 3 公理のみ．Comparator: 3 定理（`lem_relu_layer_covering`, `prop_relu_regimes`, `prop_relu_regimes_iii_lower`）を Challenge/Solution/config/README に追加し `lake build Challenge Solution` 成功（スクリプトの実行は指示により未実施）．原稿との差異は ToDraft.md「2026-09-26 (deep ReLU appendix)」（R1–R6）．
- 2026-09-26（Comparator 再実行）: 深層 ReLU 付録の 3 定理を追加した `comparator/config.json`（計 64 定理）で `./script/comparator.sh` を実行し「Your solution is okay!」（許可公理 `propext`, `Quot.sound`, `Classical.choice` のみ）を確認．`./script/generate.sh --no-pdf` も blueprint web + checkdecls 通過．`home_page/comparator.html`（61→64，新規 3 行を対応表に追加，日付・commit を更新）と `comparator/README.md`（ReLU 分の「未実行」注記を除去し 64 定理での実行結果を記録）を反映．
- 2026-09-26（付録再編第 2 段への追随，ドキュメントのみ）: 原稿の付録が同日中にさらに再編され（統一テンプレート，付録 A–M の固定順序・タイトル，`prop:implementation` の証明が J.3 に統合），Lean 側は無変更のままドキュメントを追随させた．`blueprint/src/content.tex`: 全 `\inputleanmodule`/`\inputleannode` は維持したまま，節見出しに対応する付録 A/C/D/E/F.1–F.4/G/H/I/J–M を注記し，`Examples/Readout`（readout 系: prop:global_scalar_observable, prop:linear-interpolation, cor:rkhs-readout）を Bounds 章（付録 E）直後に移動，Examples 章を J（Regimes/Implementation）→K（ReLU）→L（ChainOfThought）→M（ODE）の順に並べ替え，本文未掲載の `Bounds/EntropyDecomp(Sample)`・`Growth/ArzelaAscoli` の見出しに "Tools not printed in the manuscript" を明記．`SUMMARY.md`: 「例・応用」行を「App. J–M」に更新し，付録再編第 2 段の対応関係（A/C/D/E/F/G/H/I/J–M と Lean モジュールの対応）を記す節を新設．`comparator/README.md`・`home_page/comparator.html`: 旧「formerly App. E / App. P」の注記が，再編後の現行付録 E（Sudakov 型逆定理）と混同されないよう明記し，新しい付録 A–M の要約を追記（ラベルは全て不変）．`lake exe cache get` と `./script/generate.sh --no-pdf` を実行し，blueprint web の生成と checkdecls が通過することを確認．
